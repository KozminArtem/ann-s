...
history = network.fit(X_train_scaled_poly, y_train, verbose=0, epochs = 100, validation_split = 0)
...
plot_loss(history, string_l = "NN tanh")

plotModelResults(network, X_train=X_train_scaled_poly, X_test=X_test_scaled_poly, string = "NN tanh hour + pol:"+ str(Degree),  plot_intervals=True,\
                 y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
...
List_hour_train = range(100, 5000, 200)

MSE_tes = np.array([np.NaN] * len(List_hour_train))
MSE_tra = np.array([np.NaN] * len(List_hour_train))

MAPE_tes = np.array([np.NaN] * len(List_hour_train))
MAPE_tra = np.array([np.NaN] * len(List_hour_train))
...
for count, trsize in enumerate(List_hour_train):
    print(trsize)
    X_tra, X_te, y_tra, y_te = timeseries_train_test_split(X_all_scaled_poly, y_all, trsize)
    X_tes, XXX, y_tes, yyy = timeseries_train_test_split(X_te, y_te, 2000)  


    network = models.Sequential()
    l_feat = X_tra.shape[1]
    network.add(layers.Dense(units = 5, activation = 'tanh', input_shape=(l_feat,)))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss='mean_squared_error', optimizer= keras.optimizers.Adam(0.001), metrics=['mean_squared_error']) 

    history = network.fit(X_tra, y_tra, verbose=0, epochs = 100, validation_split = 0.3)

    MSE_tra[count] = mean_s_error(y_tra,network.predict(X_tra).flatten())
    MSE_tes[count] = mean_s_error(y_tes,network.predict(X_tes).flatten())

    MAPE_tra[count] = mean_absolute_percentage_error(y_tra,network.predict(X_tra).flatten())
    MAPE_tes[count] = mean_absolute_percentage_error(y_tes,network.predict(X_tes).flatten())
...
axis[0].plot(List_hour_train, MSE_tes, label="test_for/for", linewidth=2.0, color = "red")
axis[0].plot(List_hour_train, MSE_tra, label="train_for/for", linewidth=2.0, color = "orange")
...
# plt.figure(figsize=(15, 7))
axis[1].plot(List_hour_train, MAPE_tes, label="test_for/for", linewidth=2.0, color = "red")
axis[1].plot(List_hour_train, MAPE_tra, label="train_for/for", linewidth=2.0, color = "orange")
# plt.title("MAPE")
# plt.legend(loc="best")
# plt.tight_layout()
# plt.grid(True)
...
for count, trsize in enumerate(List_hour_train):
    ...
    X_tr, X_tes, y_tr, y_tes = timeseries_train_test_split(X_all_scaled_poly, y_all, Len_data - 2000)
    XXX, X_tra, yyy, y_tra = timeseries_train_test_split(X_tr, y_tr, Len_data - 2000 - trsize)
        


    # X_tr, X_te, y_tr, y_te = timeseries_train_test_split(X_all_scaled_poly, y_all, trsize)
    # X_tes, XXX, y_tes, yyy = timeseries_train_test_split(X_te, y_te, 2000)  
...
axis[0].plot(List_hour_train, MSE_tes, label="test_back/back", linewidth=2.0, color = "blue")
axis[0].plot(List_hour_train, MSE_tra, label="train_back/back", linewidth=2.0, color = "dodgerblue")

# plt.plot(List_hour_train, MSE_tes, label="test", linewidth=2.0, color = "red")
# plt.plot(List_hour_train, MSE_tra, label="train", linewidth=2.0, color = "green")
# plt.title("MSE_back")
# plt.legend(loc="best")
# plt.tight_layout()
# plt.grid(True)
...
axis[1].plot(List_hour_train, MAPE_tes, label="test_back/back", linewidth=2.0, color = "blue")
axis[1].plot(List_hour_train, MAPE_tra, label="train_back/back", linewidth=2.0, color = "dodgerblue")

# plt.figure(figsize=(15, 7))
# plt.plot(List_hour_train, MAPE_tes, label="test", linewidth=2.0, color = "red")
# plt.plot(List_hour_train, MAPE_tra, label="train", linewidth=2.0, color = "green")
# plt.title("MAPE_back")
# plt.legend(loc="best")
# plt.tight_layout()
# plt.grid(True)
...
for count, trsize in enumerate(List_hour_train):
    ...
    X_tr, X_tes, y_tr, y_tes = timeseries_train_test_split(X_all_scaled_poly, y_all, Len_data - 2000)
    X_tra, XXX, y_tra, yyy = timeseries_train_test_split(X_tr, y_tr, trsize)
...
axis[0].plot(List_hour_train, MSE_tes, label="test_for/back", linewidth=2.0, color = "green")
axis[0].plot(List_hour_train, MSE_tra, label="train_for/back", linewidth=2.0, color = "lime")


# plt.plot(List_hour_train, MSE_tes, label="test", linewidth=2.0, color = "red")
# plt.plot(List_hour_train, MSE_tra, label="train", linewidth=2.0, color = "green")
# plt.title("MSE_back/forward")
...
axis[1].plot(List_hour_train, MAPE_tes, label="test_for/back", linewidth=2.0, color = "green")
axis[1].plot(List_hour_train, MAPE_tra, label="train_for/back", linewidth=2.0, color = "lime")

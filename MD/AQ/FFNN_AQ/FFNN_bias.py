# Dense: use_bias = True по умолчанию,
# поэтому входной "1"  - не нужен его стоит убрать
...
def PolyScaledData(data_df = df_new, target_name = feat_target, pol_deg = Degree, TimeSplit = True, Test_size = T_size, add_hour = False, \
    ...
    X_Tr = pd.DataFrame(scaler.fit_transform(X_train_scaled_poly))
    X_Te = pd.DataFrame(scaler.transform(X_test_scaled_poly))
    list_X = X_test_scaled_poly.columns.values.tolist()
    X_Tr.set_axis(list_X, axis = 'columns', inplace=True)
    X_Te.set_axis(list_X, axis = 'columns', inplace=True)
    return X_Tr, y_train, time_train, X_Te, y_test, time_test
...
lr = LinearRegression()
lr.fit(X_train_scaled_poly, y_train)
plotModelResults(lr, X_train=X_train_scaled_poly, X_test=X_test_scaled_poly, string = "Linear",  plot_intervals=True,\
                 y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
plotCoefficients(lr, X_train=X_train_scaled_poly)
...
#     l_feat = X_train_scaled_poly.shape[1]
#     network.add(layers.Dense(units = 1, activation = 'linear', input_shape=(l_feat,)))
#     network.compile(loss='mean_squared_error', optimizer= keras.optimizers.Adam(0.01, decay = 0.005), metrics=['mean_squared_error']) 
#     history = network.fit(X_train_scaled_poly, y_train, verbose=0, epochs = 1000, validation_split = 0)
#     # plot_loss(history, string_l = "NN(ML) Adam 0.001")
#     plotModelResults(network, X_train=X_train_scaled_poly, X_test=X_test_scaled_poly, string = "NN(ML) Adam 0.005 (decay = 0.005),  Window = 1 day",  plot_intervals=True,\
#                      y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
#     keras.backend.clear_session()
...
for n_comp in range(3, 8):
    pca = PCA(n_components=n_comp, whiten = True)
    pca.fit(X_all_scaled_poly)
    
    X_train_PCA = pd.DataFrame(pca.transform(X_train_scaled_poly))
    X_test_PCA = pd.DataFrame(pca.transform(X_test_scaled_poly))
    X_all_PCA = pd.DataFrame(pca.transform(X_all_scaled_poly))

    lr = LinearRegression()
    lr.fit(X_train_PCA, y_train)
    plotModelResults(lr, X_train=X_train_PCA, X_test=X_test_PCA, string = "Linear PCA" + str(n_comp),  plot_intervals=True,\
                     y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
    # plotCoefficients(lr, X_train=X_train_PCA)

    # xgb = XGBRegressor(verbosity=0)
    # xgb.fit(X_train_PCA, y_train);

    # plotModelResults(xgb, X_train=X_train_PCA, X_test=X_test_PCA, string = "XGB PCA" + str(n_comp),  plot_intervals=True,\
    #                  y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)

    for iteration in range(Number_measure):
        network = models.Sequential()
        l_feat = X_train_PCA.shape[1]
        # print(iteration, l_feat)
        network.add(layers.Dense(units = 6, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(0.1)))
        ...
        history = network.fit(X_train_PCA, y_train, verbose=0, epochs = 3000,batch_size = 50, validation_split = 0.1)
        if iteration == Number_measure - 1:
            plot_loss(history, string_l = "NN Adam MAE" + str(n_comp), draw_val = True)
        MSE_Train[iteration] = mean_s_error(y_train,network.predict(X_train_PCA).flatten())    
        MAPE_Train[iteration] = mean_absolute_percentage_error(y_train,network.predict(X_train_PCA).flatten())
        MSE_Test[iteration] = mean_s_error(y_test,network.predict(X_test_PCA).flatten())    
        MAPE_Test[iteration] = mean_absolute_percentage_error(y_test,network.predict(X_test_PCA).flatten())
        keras.backend.clear_session()

    print(n_comp, MSE_Test.min(), MSE_Test.mean(), MSE_Test.max())
    print(n_comp, MAPE_Test.min(), MAPE_Test.mean(), MAPE_Test.max())
...
for n_comp in range(2, 6):
    ...
    fa.fit(X_all_scaled_poly)
    print(pd.DataFrame(fa.components_, columns = X_all_scaled_poly.columns))
    print("Uniqueness:")
    print(pd.Series(fa.noise_variance_, X_all_scaled_poly.columns))

    X_train_FA = pd.DataFrame(fa.transform(X_train_scaled_poly))
    X_test_FA = pd.DataFrame(fa.transform(X_test_scaled_poly))
    X_all_FA = pd.DataFrame(fa.transform(X_all_scaled_poly))

    lr = LinearRegression()
    lr.fit(X_train_FA, y_train)
    plotModelResults(lr, X_train=X_train_FA, X_test=X_test_FA, string = "Linear FA" + str(n_comp),  plot_intervals=True,\
                     y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
    plotCoefficients(lr, X_train=X_train_FA)

    # # xgb = XGBRegressor(verbosity=0)
    # # xgb.fit(X_train_FA, y_train);

    # # plotModelResults(xgb, X_train=X_train_FA, X_test=X_test_FA, string = "XGB FA" + str(n_comp),  plot_intervals=True,\
    # #                  y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
    ...
    for iteration in range(Number_measure):
        network = models.Sequential()
        l_feat = X_train_FA.shape[1]
        # print(iteration, l_feat)
        network.add(layers.Dense(units = 6, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(0.1)))
        network.add(layers.Dense(units=1, activation = 'linear'))
        network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
        history = network.fit(X_train_FA, y_train, verbose=0, epochs = 3000,batch_size = 50, validation_split = 0.1)
        if iteration == Number_measure - 1:
            plot_loss(history, string_l = "NN Adam MAE", draw_val = True)
        MSE_Train[iteration] = mean_s_error(y_train,network.predict(X_train_FA).flatten())    
        MAPE_Train[iteration] = mean_absolute_percentage_error(y_train,network.predict(X_train_FA).flatten())
        MSE_Test[iteration] = mean_s_error(y_test,network.predict(X_test_FA).flatten())    
        MAPE_Test[iteration] = mean_absolute_percentage_error(y_test,network.predict(X_test_FA).flatten())
        keras.backend.clear_session()

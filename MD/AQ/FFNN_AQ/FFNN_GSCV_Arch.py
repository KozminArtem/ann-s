...
# X_tr_tr, X_val, y_tr_tr, y_val = timeseries_train_test_split(X_train_scaled_poly, y_train, test_size=1200)
...
for iteration in range(Number_measure):
    ...
    network.add(layers.Dense(units = 1, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 1, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 1, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(learning_rate = 0.01, decay = 0.001, epsilon = 1e-01), metrics=['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000, batch_size = 50)
    # print(history)
    # plot_loss(history, string_l = "FFNN " + str(iteration), draw_val = True)
    # print(history.history.keys())

    pr_X_tr_tr = network.predict(X_tr_tr).flatten()
    pr_X_val = network.predict(X_val).flatten()

    MSE_Train[iteration] = mean_s_error(y_tr_tr,pr_X_tr_tr)    
    MAPE_Train[iteration]= mean_absolute_percentage_error(y_tr_tr,pr_X_tr_tr)
    GRE_Train[iteration] = gate_rate_error(y_tr_tr,pr_X_tr_tr)

    MSE_Test[iteration]  = mean_s_error(y_val,pr_X_val)    
    MAPE_Test[iteration] = mean_absolute_percentage_error(y_val,pr_X_val)
    GRE_Test[iteration]  = gate_rate_error(y_val,pr_X_val)
    keras.backend.clear_session()


print("epochs = 1000")
print("learning_rate = 0.01, decay = 0.001, epsilon = 1e-01")
print("layer 3 neur 1")


print("TRAIN TRAIN 1977")
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 3, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 3, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 3, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(learning_rate = 0.01, decay = 0.001, epsilon = 1e-01), metrics=['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000, batch_size = 50)
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 5, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 5, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 5, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(learning_rate = 0.01, decay = 0.001, epsilon = 1e-01), metrics=['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000, batch_size = 50)
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 7, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 7, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 7, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(learning_rate = 0.01, decay = 0.001, epsilon = 1e-01), metrics=['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000, batch_size = 50)
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 10, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 10, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(learning_rate = 0.01, decay = 0.001, epsilon = 1e-01), metrics=['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000, batch_size = 50)
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 13, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 13, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units = 13, activation = activ_func, kernel_regularizer=regularizers.L1(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(learning_rate = 0.01, decay = 0.001, epsilon = 1e-01), metrics=['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000, batch_size = 50)

...
for count, neur in enumerate(List_Neur):
    ...
    MSE_Train_Neur[count] = mean_s_error(y_train,network.predict(X_train_scaled_poly).flatten())    
    MAPE_Train_Neur[count] = mean_absolute_percentage_error(y_train,network.predict(X_train_scaled_poly).flatten())

    MSE_Test_Neur[count] = mean_s_error(y_test,network.predict(X_test_scaled_poly).flatten())    
    MAPE_Test_Neur[count] = mean_absolute_percentage_error(y_test,network.predict(X_test_scaled_poly).flatten())
...
for count, neur in enumerate(List_Neur):
    ...
    history = network.fit(X_train_scaled_poly, y_train, verbose=0, epochs = 100, validation_split = 0.3)
...
for count, neur in enumerate(List_Neur):
    ...
    history = network.fit(X_train_scaled_poly, y_train, verbose=0, epochs = 100, validation_split = 0.5)

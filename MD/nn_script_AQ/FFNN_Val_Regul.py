...
List_reg = [ -2.5, -2.0, -1.5, -1.25, -1.0, -0.75, -0.5, -0.25] 
...
X_tr_tr = X_train_scaled_poly
y_tr_tr = y_train

X_val = X_test_scaled_poly
y_val = y_test




# X_tr_tr, X_val, y_tr_tr, y_val = timeseries_train_test_split(X_train_scaled_poly, y_train, test_size=1600)
print(X_tr_tr)
print(X_val)
...
for iteration in range(Number_measure):
    ...
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-3))))
    ...
    MSE_Train[iteration] = mean_s_error(y_tr_tr,network.predict(X_tr_tr).flatten())    
    MAPE_Train[iteration]= mean_absolute_percentage_error(y_tr_tr,network.predict(X_tr_tr).flatten())
    GRE_Train[iteration] = gate_rate_error(y_tr_tr,network.predict(X_tr_tr).flatten())

    MSE_Test[iteration]  = mean_s_error(y_val,network.predict(X_val).flatten())    
    MAPE_Test[iteration] = mean_absolute_percentage_error(y_val,network.predict(X_val).flatten())
    GRE_Test[iteration]  = gate_rate_error(y_val,network.predict(X_val).flatten())
    keras.backend.clear_session()
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-2.75))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-2.5))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-2.25))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-2))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-1.75))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-1.5))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-1.25))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
for iteration in range(Number_measure):
    ...
    l_feat = X_tr_tr.shape[1]
    # print(iteration, l_feat)
    network.add(layers.Dense(units = 10, activation = activ_func, input_shape=(l_feat,), kernel_regularizer=regularizers.L2(10**(-1))))
    network.add(layers.Dense(units=1, activation = 'linear'))
    network.compile(loss=loss_func, optimizer= keras.optimizers.Adam(0.005, decay = 0.005), metrics=['mean_squared_error']) 
    history = network.fit(X_tr_tr, y_tr_tr, verbose=0, epochs = 1000,batch_size = 50)
    # if iteration == Number_measure - 1:
...
# List_GRE_L2_Min = np.copy(List_GRE_Test_Min)
# List_GRE_L2_Max = np.copy(List_GRE_Test_Max)
# List_GRE_L2_Mean = np.copy(List_GRE_Test_Mean) 

# List_MSE_L2_Std = np.copy(List_MSE_Test_Std) 
# List_MAPE_L2_Std = np.copy(List_MAPE_Test_Std)
# List_GRE_L2_Std = np.copy(List_GRE_Test_Std) 
...
#                 linestyle = '--',
#                 label = "Non Reg")   #  начертание линий
# axis[0].fill_between(List_reg, List_MSE_L2_Min,List_MSE_L2_Max,
#                 facecolor='lime',
#                 alpha = 0.5,
#                 color = 'lime',    #  цвет линий
#                 linewidth = 2,      #  ширина линий
#                 linestyle = '--',
#                 label = "L2 Reg")   #  начертание линий

# axis[0].plot(List_reg, List_MSE_non_Mean, label="Non Reg Mean", linewidth=2.0, color = "red")
# axis[0].plot(List_reg, List_MSE_L2_Mean, label="L2 Reg Mean", linewidth=2.0, color = "green")
...
#                 linestyle = '--',
#                 label = "Non Reg")   #  начертание линий
# axis[1].fill_between(List_reg, List_MAPE_L2_Min,List_MAPE_L2_Max,
#                 facecolor='lime',
#                 alpha = 0.5,
#                 color = 'lime',    #  цвет линий
#                 linewidth = 2,      #  ширина линий
...
# axis[1].plot(List_reg, List_MAPE_non_Mean, label="Non Reg Mean", linewidth=2.0, color = "red")
# axis[1].plot(List_reg, List_MAPE_L2_Mean, label="L2 Reg Mean", linewidth=2.0, color = "green")
...
#                 linestyle = '--',
#                 label = "Non Reg")   #  начертание линий
# axis[1].fill_between(List_reg, List_GRE_L2_Min,List_GRE_L2_Max,
#                 facecolor='lime',
#                 alpha = 0.5,
#                 color = 'lime',    #  цвет линий
#                 linewidth = 2,      #  ширина линий
...
# axis[1].plot(List_reg, List_GRE_non_Mean, label="Non Reg Mean", linewidth=2.0, color = "red")
# axis[1].plot(List_reg, List_GRE_L2_Mean, label="L2 Reg Mean", linewidth=2.0, color = "green")
...
# axis[0].plot(List_reg, List_MSE_non_Std, label="NonReg_Std", linewidth=2.0, color = "black")
# axis[0].plot(List_reg, List_MSE_L2_Std, label="L2Reg_Std", linewidth=2.0, color = "red")
# axis[0].set_title("MSE, Hidden_Tanh 10, Std")
# axis[0].legend(loc="best")

# axis[1].plot(List_reg, List_MAPE_non_Std, label="NonReg_Std", linewidth=2.0, color = "black")
# axis[1].plot(List_reg, List_MAPE_L2_Std, label="L2Reg_Std", linewidth=2.0, color = "red")
# axis[1].set_title("MAPE, Hidden_Tanh 10, Std")
# axis[1].legend(loc="best")
...
# axis[1].plot(List_reg, List_GRE_non_Std, label="NonReg_Std", linewidth=2.0, color = "black")
# axis[1].plot(List_reg, List_GRE_L2_Std, label="L2Reg_Std", linewidth=2.0, color = "red")
# axis[1].set_title("GRE, Hidden_Tanh 10, Std")
# axis[1].legend(loc="best")

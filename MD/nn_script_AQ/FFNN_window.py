# Анализ разделения данных на Window, Delta 
# Построены матрицы корреляции для новых признаков
# Замена признака mean CO(GT) (hour) на mean R_CO(hour)
...
All_scaled_poly = X_all_scaled_poly
All_scaled_poly['CO'] = y_all


print(All_scaled_poly)
All_scaled_poly = All_scaled_poly.dropna().drop(['1'], axis=1)
print(All_scaled_poly)
plt.subplots(sharey = True, figsize = (12, 12))
c_m_pear = All_scaled_poly.corr(method='pearson')
...
for j in range(X_all_scaled_poly.shape[0]):
    if j > 23:
        Window[j] = sum(All_scaled_poly['CO'][alpha] for alpha in range(j-23, j+1))/24
    else:
        Window[j] = sum(All_scaled_poly['CO'][alpha] for alpha in range(j+1))/(j+1.0)    
All_scaled_poly['W_CO'] = Window
All_scaled_poly['D_CO'] = All_scaled_poly['CO'] - All_scaled_poly['W_CO'] 
...
print(All_scaled_poly)
plt.subplots(sharey = True, figsize = (12, 12))
c_m_pear = All_scaled_poly[['W_CO','WR_CO','WR_NM','W_T']].corr(method='pearson')
sns.heatmap(c_m_pear, annot=True)
plt.title("Pearson correlation, Window 24 hours", fontsize=22)
plt.savefig('fig_NN/W_Pearson.png')



plt.subplots(sharey = True, figsize = (12, 12))
c_m_pear = All_scaled_poly[['D_CO','DR_CO','DR_NM','D_T', 'CO_hour']].corr(method='pearson')
sns.heatmap(c_m_pear, annot=True)
plt.title("Pearson correlation, Delta 24 hours", fontsize=22)
plt.savefig('fig_NN/D_Pearson.png')




plt.figure(figsize = (16,10))
plt.plot(time_all, All_scaled_poly['D_T'], label = 'D_T')
plt.plot(time_all, All_scaled_poly['CO_hour'], label = 'CO_hour')
plt.plot(time_all, All_scaled_poly['DR_CO'], label = 'DR_CO')
plt.plot(time_all, All_scaled_poly['DR_NM'], label = 'DR_NM')
plt.plot(time_all, All_scaled_poly['D_CO'], label = 'D_CO')

plt.title("Delta 24 hour")
plt.legend(loc="best")
plt.tight_layout()
plt.grid(True)
...
plt.figure(figsize = (16,10))
plt.plot(time_all, All_scaled_poly['W_T'], label = 'W_T')
plt.plot(time_all, All_scaled_poly['WR_CO'], label = 'WR_CO')
plt.plot(time_all, All_scaled_poly['WR_NM'], label = 'WR_NM')
plt.plot(time_all, All_scaled_poly['W_CO'], label = 'W_CO')
...
WX_train = X_train_scaled_poly[['WR_CO','WR_NM','W_T']]
WX_test = X_test_scaled_poly[['WR_CO','WR_NM','W_T']]
Wy_train = All_scaled_poly['W_CO'].iloc[:T_size]
Wy_test = All_scaled_poly['W_CO'].iloc[T_size:]


lr = LinearRegression()
lr.fit(WX_train, Wy_train)
plotModelResults(lr, X_train=WX_train, X_test=WX_test, string = "Window features, 24 hours",  plot_intervals=True,\
                 y_train = Wy_train, y_test = Wy_test, time_train = time_train, time_test = time_test, plot_diff = False)
plotCoefficients(lr, X_train=WX_train)

WPred = lr.predict(WX_test).flatten()
print(len(WPred))



DX_train = X_train_scaled_poly[['DR_CO','DR_NM','D_T','CO_hour']]
DX_test = X_test_scaled_poly[['DR_CO','DR_NM','D_T','CO_hour']]
Dy_train = All_scaled_poly['D_CO'].iloc[:T_size]
Dy_test = All_scaled_poly['D_CO'].iloc[T_size:]

lr = LinearRegression()
lr.fit(DX_train, Dy_train)
plotModelResults(lr, X_train=DX_train, X_test=DX_test, string = "Delta features, 24 hours",  plot_intervals=True,\
                 y_train = Dy_train, y_test = Dy_test, time_train = time_train, time_test = time_test, plot_diff = False)
plotCoefficients(lr, X_train=DX_train)

DPred = lr.predict(DX_test).flatten()
print(len(DPred))

SPred = WPred + DPred
print(len(SPred))

print(mean_absolute_percentage_error(y_test.values,SPred))
print(mean_s_error(y_test.values,SPred))
print(gate_rate_error(y_test.values,SPred))


lr = LinearRegression()
lr.fit(X_train_scaled_poly, y_train)
plotModelResults(lr, X_train=X_train_scaled_poly, X_test=X_test_scaled_poly, string = "All features",  plot_intervals=True,\
                 y_train = y_train, y_test = y_test, time_train = time_train, time_test = time_test, plot_diff = False)
plotCoefficients(lr, X_train=X_train_scaled_poly)

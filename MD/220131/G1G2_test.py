...
feat_float = list(df_src_G1G2.columns)
del feat_float[0]
...
df_src_G1G2['datetime'] = df_src_G1G2['date'].map(lambda x: datetime.datetime.strptime(str(x), "%Y-%m-%d %H:%M:%S"))
...
df_src_diffmean['datetime'] =  df_src_G1G2['datetime']  


for collumn in feat_float:
    df_src_diffmean[collumn] = df_src_G1G2[collumn] - df_src_G1G2[collumn].mean()
